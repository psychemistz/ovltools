...
ovl_kde <- function(x1, x2, n = 1024) {
    ...
    OVL = d_y1 %>% dplyr::mutate(y1 = y) %>% left_join(d_y2 %>% mutate(y2 = y), by = "x") %>% dplyr::select(x, y1, y2) %>% group_by(x) %>% 
        summarise(OVL = min(y1, y2), OVA = max(y1, y2), .groups = "drop") %>% ungroup() %>% summarise(total = sum(OVL)/sum(OVA), .groups = "drop") %>% 
