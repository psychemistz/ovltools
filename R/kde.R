## This code is adopted from overlapping paper.
## Reference: Pastore, M.,Calcagnì, A. (2019). Measuring Distribution Similarities Between Samples: A
## Distribution-Free Overlapping Index. Frontiers in Psychology, 10:1089. URL:
...
## overlap function in overlapping package provides distribution overlap based on kernel density estimation (KDE).
## The original package used base density function to perform KDE.
...
ovl_kde <- function(x1, x2, n=1024){
  ...
  OVL = d_y1 %>% dplyr::mutate(y1=y) %>% left_join(d_y2 %>% mutate(y2=y), by='x') %>%
    ...
    summarise(total = sum(OVL)/sum(OVA), .groups="drop") %>% unlist(use.names=FALSE)
