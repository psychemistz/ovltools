...
RvMat = data.frame(dx, prob_df, cum_df, rv, kde, histogram, distfit)
RvMat %>% gather(method, value, -dx) %>% ggplot(aes(x=dx, y=value, col=method)) +
  geom_point() +
  theme_bw() +
  xlab("Deviation") +
  ylab("value")
...
library(overlapping)
distfit = histogram = kde = kde_by_package = rv_from_src = rv = c()
for(i in 1:1000){
  x1 = rnorm(1000)
  x2 = x1 + 1
  tmp_rv = ovl_knn(x1, x2, k=5)
  ...
  rv_from_src = c(rv_from_src, GRvalue(c(x1,x2), c(rep(0,length(x1)), rep(1,length(x2)))))
  histogram = c(histogram, tmp_hist)
  distfit = c(distfit, tmp_distfit)
}
...
RvMat = data.frame(prob_df, cum_df, rv, rv_from_src, distfit, kde, kde_by_package, histogram)
